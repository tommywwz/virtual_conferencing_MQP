...
if __name__ == '__main__':
    ...
    class ClientThread(threading.Thread):
        ...
        def run(self):
            ...
            expected_size = 0
            while True:
                data, recv_addr = self.client.recvfrom(640 * 480 * 3)
                if expected_size == 0:
                    expected_size = int(data[:10].decode('utf-8').strip())
                    data = data[10:]
                frame += data
    ...
    while True:
        _, address = server.recvfrom(640 * 480 * 3)
        client = ClientThread(address, server)
        client.start()
